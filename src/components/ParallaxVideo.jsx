  ...
  useGSAP(()=>{
    ...
    const tl = gsap.timeline({
        scrollTrigger: {
          ...
          start: "top top",
          end: "+=" + videos.length * window.innerHeight, // one viewport per video
          scrub: true,
          pin: true,
        },
      });
      ...
      videos.forEach((video, i) => {
        if (i < videos.length - 1) {
          tl.to(video, { opacity: 0, duration: 1 }, i + 1); // fade out current
          tl.to(videos[i + 1], { opacity: 1, duration: 1 }, i + 1); // fade in next
        }
      });
    }, containerRef);
    return () => ctx.revert();
